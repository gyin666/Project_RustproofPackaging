            ...
            for (int i = 0; i < col1.Count; i++)
            {
                ...
                if (materialObj != null)
                {
                    ...
                    tmpSQL0.AppendFormat(@"/*dialect*/ SELECT SUM(E.FREALQTY) INNUM,
                                                            ...
                                                            SUM(F_SCFG_DECIMAL2) JIANNUM,
                                                            SUM(F_SCFG_DECIMAL4) M2NUM,
                                                            SUM(F_SCFG_DECIMAL6) ZHANGNUM,
                                                            SUM(F_SCFG_DECIMAL8) XIANGNUM
                                                            FROM T_PRD_INSTOCKENTRY E 
                                                            WHERE E.FMOBILLNO IN (SELECT FBILLNO FROM T_PRD_MO WHERE F_SCFG_TEXT = '{0}') AND E.FMATERIALID = {1} ", saleBillNo, materialId);
                    ...
                    if (col0 != null && col0.Count > 0)
                    {
                        ...
                        querySQL0.AppendFormat(@"/*dialect*/ SELECT SUM(FREALQTY) OUTNUM,
	                                   ...
	                                   SUM(F_SCFG_XIANGNUM) XIANGNUM,
	                                   SUM(F_SCFG_JIANNUM) JIANNUM
                                FROM T_SAL_OUTSTOCKENTRY E
                                LEFT JOIN T_SAL_OUTSTOCKENTRY_R ER
                                ON E.FID = ER.FID
                                WHERE ER.FSOORDERNO = '{0}' AND E.FMATERIALID = {1} ", saleBillNo, materialId);
                            ...
                            if (!String.IsNullOrWhiteSpace(packNo))
                            {
                                ...
                                if (col2 != null && col2.Count > 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        this.View.Model.SetValue("F_SCFG_XIANGNUM", Convert.ToDouble(col0[0]["XIANGNUM"]) - Convert.ToDouble(col00[0]["XIANGNUM"]), i);
                                        this.View.Model.SetValue("F_SCFG_JIANNUM", Convert.ToDouble(col0[0]["JIANNUM"]) - Convert.ToDouble(col00[0]["JIANNUM"]), i);
                                    }
                                }
                            }
                            else
